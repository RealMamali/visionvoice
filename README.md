# **visionvoice**
English | [Русский](https://github.com/RealMamali/visionvoice)

## Description
---------------------------------
**visionvoice** - is a Software Development Kit (SDK) for implementing voice communication systems in the Pawn language for SA:MP servers.
...
##### Automatically
---------------------------------
1. In order to download the installer, head over to [the `releases` page](https://github.com/RealMamali/visionvoice) and choose the desired version of the plugin.
...
##### Manually
---------------------------------
1. Head over [the `releases` page](https://github.com/RealMamali/visionvoice) and download the archive with the desired client version.
2. Extract the archive to your GTA San Andreas folder.

#### For developers
---------------------------------
1. Download from [the `releases` page](https://github.com/RealMamali/visionvoice) the desired version of the plugin for your platform.
2. Unpack the archive to the root directory of the server.
3. Add to the *server.cfg* server configuration file the line *"plugins visionvoice"* for *Win32* and *"plugins visionvoice.so"* for *Linux x86*. **(If you have a Pawn.RakNet plugin be sure to place visionvoice after it)**

## Usage
---------------------------------
To get started using the plugin, read the documentation that comes with the server side. To do this, open the *visionvoice.chm* file using the Windows reference. **(If the documentation does not open, right-click on the documentation file, then Properties -> Unblock -> OK)**
...
public OnPlayerConnect(playerid)
{
    ...
    {
        SendClientMessage(playerid, -1, "Could not find plugin visionvoice.");
    }
...
```sh
git clone https://github.com/RealMamali/visionvoice.git
cd visionvoice
```

### Windows (Client/Server)
---------------------------------
To compile the client side of the plugin, you need the *DirectX SDK*. By default, the client part is compiled for version **SA: MP 0.3.7 (R1)**, but you can also explicitly tell the compiler the version for the build using the **SAMP_R1** and **SAMP_R3** macros. In order to build the client and server parts of the plugin for the *Win32* platform, open the *visionvoice.sln* project in MS Visual Studio 2019 and compile it:
